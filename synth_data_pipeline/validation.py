...
from .schemas import AlertsOutput, GeneratedComment
...
class Validator:
    ...
    def validate_single(
        ...
        target_labels = set(generated.target_labels)
    ...
    def _default_label_extractor(self, result: BaseModel) -> Set[str]:
        if isinstance(result, AlertsOutput):
            return {a.alert_type for a in result.alerts} if result.alerts else set()
        return set()
    ...
    def summary(self, results: List[ValidationResult]) -> Dict:
        ...
        return {
            ...
            "avg_f1": avg_f1,
            "commonly_missed_labels": dict(
                sorted(all_missed.items(), key=lambda x: -x[1])[:10]
            ),
